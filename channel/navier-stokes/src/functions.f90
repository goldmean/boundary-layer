...
CONTAINS
    ...
    SUBROUTINE BOUNDARY_CONDITIONS(UCELL, VCELL, PCELL)
        ...
        UCELL(:, 0) = UCELL(:, 1)  ! SYMMETRY
        VCELL(:, 0) = - VCELL(:, 1)
        PCELL(:, 0) = PCELL(:, 1)

        UCELL(NI, :) = UCELL(NI - 1, :)  ! OUTLET
        VCELL(NI, :) = VCELL(NI - 1, :)
        PCELL(NI, :) = P0
    ...
    SUBROUTINE NUMERICAL_FRICTION(UCELL, CF)
        ...
        DO I = 1, NI
            CF(I) = 2.0 * MU * (UCELL(I, NJ - 1) - UCELL(I, NJ)) / &
                YSTEP / (R0 * MAXVAL(UCELL(I, :)) ** 2)  ! SECOND ORDER
        END DO
    ...
    SUBROUTINE CREATE_FILES(XNODE, YNODE, UCELL, VCELL, PCELL, RE, CF, BL)
        IMPLICIT NONE

        DOUBLE PRECISION, INTENT(IN), DIMENSION(:) :: RE, CF, BL
        DOUBLE PRECISION, INTENT(IN), DIMENSION(:, :) :: XNODE, YNODE
        DOUBLE PRECISION, INTENT(IN), DIMENSION(0:, 0:) :: UCELL, VCELL, PCELL
